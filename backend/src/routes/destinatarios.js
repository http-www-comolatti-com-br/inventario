...
router.post('/', auth, async (req, res) => {
  try {
    const { nome_completo, setor, filial, ativo, observacoes } = req.body;
    if (!nome_completo) return res.status(400).json({ error: 'Nome completo é obrigatório.' });
    const result = await pool.query(
      `INSERT INTO destinatarios (nome_completo, setor, filial, ativo, observacoes)
       VALUES ($1, $2, $3, $4, $5) RETURNING *`,
      [nome_completo, setor || null, filial || null, ativo !== false, observacoes || null]
    );
...
router.put('/:id', auth, async (req, res) => {
  try {
    const { nome_completo, setor, filial, ativo, observacoes } = req.body;
    const result = await pool.query(
      `UPDATE destinatarios SET nome_completo=$1, setor=$2, filial=$3, ativo=$4, observacoes=$5, atualizado_em=NOW()
       WHERE id=$6 RETURNING *`,
      [nome_completo, setor || null, filial || null, ativo, observacoes || null, req.params.id]
    );
    if (result.rows.length === 0) return res.status(404).json({ error: 'Destinatário não encontrado.' });
    res.json(result.rows[0]);
  } catch (err) {
